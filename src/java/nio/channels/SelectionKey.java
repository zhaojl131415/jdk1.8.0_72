 ...
 * @see Selector
 *
 * SelectionKey是一个抽象类, 表示SelectableChannel在Selector中注册的标识. 每个Channel向Selector注册时, 都会创建一个SelectionKey
 * SelectionKey将Channel向Selector建立了关系, 并维护了Channel事件
 * 可以通过cancel方法取消SelectionKey, 取消后不会立即从Selector中移除, 而是添加到cancelledKeys中, 在下一次Selector操作时移除,
 * 所以在调用某个SelectionKey, 需要调用isValid进行验证
 */
...
public abstract class SelectionKey {
     ...
     * 返回为其创建此键的通道。 即使取消了键，此方法也将继续返回频道。
     *
